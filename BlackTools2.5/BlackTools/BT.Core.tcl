...
set black(tempdir) "$black(dirname)/BlackTools/FILES/temp"
...
set black(temp_banner) "$black(dirname)/BlackTools/FILES/temp/banner"
set black(temp_motd) "$black(dirname)/BlackTools/FILES/temp/motd"
...
set black(extra_str) {
	...
	antichanflood-reason private-reason clonescan-reason dr-reason vr-reason n-reason
	b-reason id-reason w-reason w-message spam-reason bw-reason bot-reason gag-reason stick-reason troll-reason
	...
	dr-bantime vr-bantime n-bantime b-bantime id-bantime spam-bantime bw-bantime
	...
	antichanflood-banmask private-banmask dr-banmask vr-banmask stick-banmask
...
set black(extra_flag) {
	...
	antijoinflood antichanflood xtools antibadchan anunt limit clonescan seen autoop
	autovoice leave topwords dontop dontdeop private silent quote note reportnick invisible
	showhandle showid showcount showtime showurl next voiceonmsg autotopic greet xonly
...
set black(validcmds) "o v ho man version mode cycle say act i activ t myset h r msg add adduser set userlist info delacc del auto chuser s us addhost delhost b black stick dr bot n id spam bw vr gag ungag troll ub sb banlist banstats k w purge set unset show ignore enable disable addchan delchan suspend unsuspend channels die jump save rehash nick uptime status cp chat next noidle skip helped anunt bt idle limit seen clonescan egg badchan securemode private tcl guestnick greet leave topic timer topwords next q note"
...
if {[matchattr $users -|M $chan] && [matchattr $hand -|M $chan]} {
	...
	return 1
} elseif {[matchattr $users m|- $chan] && [matchattr $hand o|- $chan] && (![matchattr $hand nm|- $chan])} {
	return 1
} elseif {[matchattr $users n|- $chan] && [matchattr $hand o|- $chan] && (![matchattr $hand nm|- $chan])} {
	return 1
...
if {[setting:get $chan showtime] && $bantime != "0"} {
	set show_reason "$show_reason \[ban-time: [return_reason_time [expr $reason_time - [unixtime]]]\]"
} else {
...
if {$bl == "1"} {
	set show_reason "Shield: ($gethand) blacklisted -- ($black(say.$getlang.gl.reason): $reason)"
} else {
	set show_reason "($gethand) $reason"
	}
} else {
if {$bl == "1"} {
	set show_reason "Shield: blacklisted -- ($black(say.$getlang.gl.reason): $reason)"
} else {
...
if {[onchan $black(chanserv) $chan] && $xban == "1"} {
	...
	set reply [string map [array get replace] $black(say.$getlang.gl.xonlyban)]
if {$prv == "prv"} {
...
if {$xonly == "1"} {
	...
	3 {
	banmethod_action:ban $nick $chan $host $reason $xban $xonly
...
if {$get == "$num"} {
	...
	banmethod_action:ban $nick $c $host $reason $xban $xonly
	blacktools:addban $nick $host "$type:$said" $c "" $bantime "BAN" "0" "0" $thereason "0" "" "" "" 0 $num
	}
}
if {$mode == "1"} {
	putserv "MODE $chan +$chanmode_type" -next ;utimer $chanmode_time [list putserv "MODE $chan -$chanmode_type"]
	}
	banmethod_memory:remove $host $chan $variable_name
}
	4 {
...
proc banmethod_action:ban {nick chan host reason xban xonly} {
	global black
		chanserv:ignore_remove
if {$xonly == "1"} {
	set getxtime [setting:get $chan xbantime]
...
if {[string match -nocase "*.mibbit.com*" $host]} {
...
if {![string equal -nocase "$cmd" "black"] && ![string equal -nocase "$cmd" "troll"]} {
...
if {$bl == "1"} {
if {[setting:get $chan showhandle]} {
	set show_reason "Shield: ($gethand) blacklisted -- ($black(say.$getlang.gl.reason): $read_reason)" 
} else {
	set show_reason "Shield: blacklisted -- ($black(say.$getlang.gl.reason): $read_reason)"
	}
if {[setting:get $chan showid]} {
	set show_reason "$show_reason \[id: $num\]"	
	}
...
if {$bl == "1"} {
if {[setting:get $chan showhandle]} {
	set show_reason "Shield: ($read_handle) blacklisted -- ($black(say.$getlang.gl.reason): $read_reason)" 
} else {
	set show_reason "Shield: blacklisted -- ($black(say.$getlang.gl.reason): $read_reason)"
	}
if {[setting:get $chan showid]} {
...
if {$bancheck != ""} {
	return [lindex [split $bancheck] 6]
	}
	return
}
...
if {[isvoice $nick $chan]} {
	...
	pushmode $chan +b $read_host
	puthelp "PRIVMSG $chan :[string map [array get replace] $black(say.$getlang.gag.1)]"
	continue
		}
	}	
}
...
if {[string match -nocase $real_read_host $uhost]} {
	...
	set replace(%chan%) $chan
	puthelp "PRIVMSG $chan :[string map [array get replace] $black(say.$getlang.gag.5)]"
			}
		}
	}
}
...
if {[setting:get $chan showtime] && ($expire != "0") && ($cmd != "bot")} {
	set show_reason "$show_reason \[ban-time: $expire\]"
}
...
if {[info exists black(logged)]} {
	putlog "\[BlackTools\] Still logged.."
	return
}
	chanserv:ignore_remove
...
if {$total == "total"} {
	...
	set gettext [lrange [split $text] 1 end]
...
} elseif {[string equal -nocase "***" $lastbind]} { set charbind "$host" }
if {$charbind == ""} {
	set charbind $firstchar
if {[validuser $hand]} {
	set getchar [getuser $hand XTRA MYCHAR]
