...
global black lastbind
	...
	set type 0
	set chan1 "$chan"
if {[regexp {^[&#]} $level] && [matchattr $hand nmo|AM $level]} {
	set chan "$level"
	set level [lindex [split $arg] 1]
	set args [lrange [split $arg] 2 end]
}
...
if {![onchan $user $chan] && ![validuser $user]} { 
	blacktools:tell $nick $host $hand $chan $chan1 gl.usernotonchan [split $user]
	return
}
...
if {[matchattr $hand n|- $chan] && [matchattr $user m|- $chan]} {
	blacktools:tell $nick $host $hand $chan $chan1 gl.isowner "$show_user [blacktools:getlevelname 2 $hand]"
	return
}
if {[matchattr $hand n|- $chan] && [matchattr $user o|- $chan]} {
	blacktools:tell $nick $host $hand $chan $chan1 gl.isowner "$show_user [blacktools:getlevelname 8 $hand]"
	return
}
if {[matchattr $hand m|- $chan] && [matchattr $user o|- $chan]} {
	blacktools:tell $nick $host $hand $chan $chan1 gl.isowner "$show_user [blacktools:getlevelname 8 $hand]"
	return
}
...
if {[validuser $user]} { 
	chattr $user o-|-qolSHOAMV $chan
	chattr $user $flags
	setuser $user XTRA CHANMODIF(GLOBAL) $time:$hand
	blacktools:tell $nick $host $hand $chan $chan1 add.19 "$show_user [blacktools:getlevelname 2 $hand]"
} else {
	adduser $user $uhost
	set user [check:handle $user $nick $host $hand $chan $chan1]
	chattr $user -|-qolSHOAMV $chan
...
if {[regexp {^[&#]} $level] && [matchattr $hand nmo|MAOV $level]} {
	set chan "$level"
	set level [lindex [split $arg] 1]
if {![validchan $chan]} {
	blacktools:tell $nick $host $hand $chan $chan1 gl.novalidchan none
...
if {[regexp {^[&#]} $user] && [matchattr $hand nmo|MASO $user] && ([lindex [split $arg] 1] == "")} {
	set chan $user
	info:process:chan $nick $host $hand $chan $chan1
	return
}

if {[regexp {^[&#]} $user] && [matchattr $hand nmo]} {
	set chan "$user"
...
if {$black(abuse:$chan) == "1"} {
	set status "$black(say.$getlang.info.29)"
	} elseif {$black(abuse:$chan) == "3"} {
	set status "$black(say.$getlang.info.27)"
	} elseif {$black(abuse:$chan) == "2"} {
	set status "$black(say.$getlang.info.28)"
	} elseif {$black(abuse:$chan) == "4"} {
	set status "$black(say.$getlang.info.26)"
	} elseif {$black(abuse:$chan) == "5"} {
	set status "$black(say.$getlang.info.35)"
	}
} else {
	set status "$black(say.$getlang.info.22)"
	}
}
...
if {[info exists black(abuse:$chan)]} {
if {$black(abuse:$chan) == "1"} {
	set status "$black(say.$getlang.info.33)"
	} elseif {$black(abuse:$chan) == "3"} {
	set status "$black(say.$getlang.info.31)"
	} elseif {$black(abuse:$chan) == "2"} {
	set status "$black(say.$getlang.info.32)"
	} elseif {$black(abuse:$chan) == "4"} {
	set status "$black(say.$getlang.info.30)"
	} elseif {$black(abuse:$chan) == "5"} {
	set status "$black(say.$getlang.info.34)"
	} 
} else {
	set status "$black(say.$getlang.info.23)"
		}
	}
}
...
if {![info exist found_it]} {
if {![botisop $chan]} {
	set status "$black(say.$getlang.info.24)"
		}
	}
if {$status != "OK"} {
...
if {[getuser $user INFO] != ""} {
	blacktools:tell $nick $host $hand $chan $chan1 info.3 "[split [getuser $user INFO]]"
}
...
proc getsettings {user} {
	global black
	set output ""
	set settings "lang OUTPUT_TYPE autoinvite mychan mychar OUTPUT_COLORS"
...
global black lastbind
	...
	set chan1 "$chan"
	set user [lindex [split $arg] 0]
	set suspend_time [lindex [split $arg] 1]
	set suspend_reason [join [lrange [split $arg] 2 end]]
if {![regexp {^[0-9]} $suspend_time]} {
	set suspend_reason [join [lrange [split $arg] 1 end]]
}
if {[regexp {^[&#]} $user] && [matchattr $hand nmo]} {
	set chan "$user"
	set user [lindex [split $arg] 1]
...
if {$return == "0"} {
	...
	us:process $nick $host $hand $chan $chan1 $user $type
}
