...
set black(seen_details) "$black(dirname)/BlackTools/FILES/$username.seen_details.txt"
...
set black(files) "$black(s_file) $black(status_file) $black(seen_file) $black(seen_details) $black(uptime_file) $black(bans_file) $black(join_file) $black(extra_file) $black(add_file) $black(quote_file) $black(notes_file)"
...
set black(extra_flag) {
	...
	idleop idlehalfop idlevoice vprotect oprotect hoprotect badquitpart quitpartcolor quitpartmsgflood badhost nickflood guestnick seenreply seenlastmsg
...
if {[string match -nocase $h $fullhost] || $cidr == "1"} {
...
if {$black(chanremove_all) == "0"} {
	...
	set files "$black(s_file) $black(seen_file) $black(seen_details) $black(bans_file) $black(join_file) $black(extra_file) $black(add_file) $black(quote_file)"
...
if {[string equal -nocase $enc_chan $chan] && [string match -nocase $t $type] && [string match -nocase $w $word] && ![regexp {^[+]} $w] && ![regexp {^[+]} $word]} {
	set found_it 1
		} elseif {[string equal -nocase $enc_chan $chan] && [string match -nocase $t $type] && [string match -nocase $w $word] && [regexp {^[+]} $w] && [regexp {^[+]} $word]} {
	set found_it 1	
		}
	}
	return $found_it
