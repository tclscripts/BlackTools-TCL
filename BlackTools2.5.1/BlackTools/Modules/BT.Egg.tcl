...
voiceme-showtime {
	...
	set return_time [time_return_minute $result]
if {$return_time == "-1"} {
	set return_time $black(voiceme:msgtime)
	set return_time [time_return_minute $return_time]
}
if {$result == ""} {
	set get_info [config:getinfo $tcl_config "set black(voiceme:msgtime) \"*\""]
	egg:show $type $get_info $unick $host $hand $chan $chan1 $type2
	return
	}
	set error [config:save $tcl_config "set black(voiceme:msgtime) \"*\"" "set black(voiceme:msgtime) \"$result\""]
if {$error == "0"} {
	blacktools:tell $unick $host $hand $chan $chan1 egg.4 "voiceme-showtime"
	return
	}
	blacktools:tell $unick $host $hand $chan $chan1 egg.3 "voiceme-showtime"
	foreach tmr [timers] {
if {[string equal -nocase "voiceonmsg:timer" [lindex $tmr 1]]} {
	killtimer [lindex $tmr 2]
	set found_timer 1
			}	
		}
if {$found_timer == "1"} {
	timer $return_time voiceonmsg:timer
}
	rehash
}
