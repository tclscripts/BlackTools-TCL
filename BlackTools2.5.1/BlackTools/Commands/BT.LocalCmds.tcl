...
if {$type == "2"} {
	...
	set local_cmds "h omsg o v ho man version mode cycle broadcast say act i activ t myset b black stick dr bot n id spam bw vr gag sb banlist banstats k w anunt bt badchan idle limit seen clonescan securemode private guestnick greet leave topic next helped noidle skip topwords q note report"
...
if {[string match "*securemode:timer*" [join [lindex $tmr 1]]]} {
	...
	putserv "PRIVMSG $chan1 :[string map [array get replace] $reply2]"
...
foreach u [lsort -integer -decreasing [array names activlist]] {
	set counter [expr $counter + 1]
if {[lsearch -exact [string tolower $activlist($u)] [string tolower $show_user]] > -1} {
	set place $counter
	}
}
	return $place:$all_count
